...
?>
<!-- BLOC CALCUL CLASSIQUE (planète de référence + graphiques & export) -->
<div class="card shadow-sm mb-4">
  <div class="card-body">
    <h5 class="mb-3">Planète de référence (tableau & graphiques)</h5>
    <form method="post" novalidate id="refForm">
      <div class="row g-3">
        <div class="col-md-4">
          <label class="form-label">Demi-grand axe \(a\)</label>
          <div class="input-group">
            <input type="text" name="a_value" id="ref_a_value" class="form-control" placeholder="ex: 0.387 (UA) ou 5.8e10 (m)"
                   value="<?= esc($_POST['a_value'] ?? '0.387') ?>">
            <select name="a_unit" id="ref_a_unit" class="form-select" style="max-width:120px">
        ...
        <div class="col-md-4">
          ...
          <input type="text" name="e" id="ref_e" class="form-control" placeholder="ex: 0.2056 (Mercure)"
                 value="<?= esc($_POST['e'] ?? '0.2056') ?>">
        </div>
        <div class="col-md-4">
          <label class="form-label">Masses stellaires (mêmes que l’animation)</label>
          <textarea name="masses" id="ref_masses" class="form-control" rows="3"><?= esc($_POST['masses'] ?? '1') ?></textarea>
          <div class="mt-2">
            <label class="form-label">Unité</label>
            <select name="m_unit" id="ref_m_unit" class="form-select">
...
<?php if ($result): ?>
  <div class="card shadow-sm mb-4">
    <div class="card-body">
      <h5 class="mb-3">Résultats (planète de référence)</h5>
      <div class="row">
        <div class="col-md-6">
          <ul class="list-unstyled">
            ...
            <li>Σ masses (étoiles) : <strong><?= number_format($result['mtot_kg'], 3, ',', ' ') ?></strong> kg</li>
            <li>Paramètre \( \mu \) : <strong><?= sprintf('%.6e',$result['mu']) ?></strong> m³/s²</li>
          </ul>
        </div>
        <div class="col-md-6">
          <ul class="list-unstyled">
      ...
      <hr>
      <h6>Précession (approx. Schwarzschild)</h6>
      <ul class="list-unstyled">
  ...
  <!-- GRAPHIQUES (hauteur fixée 400px) + EXPORTS -->
  <div class="card shadow-sm mb-4">
    <div class="card-body">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
        <h5 class="mb-0">ω(t) sur 1 siècle</h5>
      ...
      <div class="mt-3">
        <h6 class="mb-2">ω(t) en degrés</h6>
        <canvas id="omegaChartDeg" class="fixed-chart"></canvas>
      </div>
      <div class="mt-4">
        <h6 class="mb-2">ω(t) en arcsecondes</h6>
        <canvas id="omegaChartArcsec" class="fixed-chart"></canvas>
      </div>
    </div>
  </div>
  ...
  <div class="card shadow-sm mb-4">
    <div class="card-body">
      <h5 class="mb-3">Évolution annuelle (0→100 ans)</h5>
      <div class="table-responsive" style="max-height:480px;">
        <table class="table table-sm table-striped align-middle" id="evolutionTable">
